...
public class Laptop extends Node {
	 ...
	 * 
	 */
	Laptop(Terminal terminal, int port, Integer[] availRouters, Integer[] smartPhoneDests, String dstHost) {
		try {
...
//			socket= new DatagramSocket(port);
			...
			for(int i=0; i< routers.length; i++){
				sockets.add(new DatagramSocket(routers[i]));
			}
			for(int i =0; i<smartPhoneDests.length;i++){
				sockets.add(new DatagramSocket(smartPhoneDests[i]));
			}

			// InetAddresses
			for(int i=0;i<availRouters.length;i++){
				tempAddress = new InetSocketAddress( dstHost,availRouters[i]);
				routerAdrs [i] = tempAddress;
			}
			for(int i=0;i< smartPhoneDests.length;i++){
				tempAddress = new InetSocketAddress( dstHost,smartPhoneDests[i]);
	...
	public synchronized void updateTable(RoutingTable table){
		...
		for(int i = 0; i < table.dest.size(); i++){
			...
			if(!this.routTable.dest.contains(newDest)){
				...
				this.routTable.cost.add(newDestCost + 1);
				this.routTable.nextNode.add(table.routerID);
				// this.routTable.nextNode.add(currentNextNode);
				System.out.println("Added a new destination!");
			}
			int thisDest = this.routTable.dest.get(this.routTable.dest.indexOf(newDest));
			int thisCost = this.routTable.cost.get(this.routTable.cost.indexOf(newDestCost));
